...
def create_model(n_fft, width):
    ...
    conv1_gyro = keras.layers.TimeDistributed(
        keras.layers.Conv1D(filters=int(width * 16), kernel_size=(3,), strides=(1,), input_shape=[N_GYRO, 3],
                            activation=tf.nn.relu))(
        input_gyro)
    conv1_lin_accel = keras.layers.TimeDistributed(
        keras.layers.Conv1D(filters=int(width * 16), kernel_size=(3,), strides=(1,), input_shape=[N_LIN_ACCEL, 3],
