...
def create_model(n_fft, width):
    ...
    spatial_attention = keras.layers.Dense(int(width * 20), activation=tf.nn.softmax)(time_distributed)
    spatial_attention_result = keras.layers.multiply([time_distributed, spatial_attention])

    # Apply temporal attention
    temporal_attention = keras.layers.Dense(1, activation=tf.nn.tanh)(spatial_attention_result)
    temporal_attention = keras.layers.Flatten()(temporal_attention)
    temporal_attention = keras.layers.Activation('softmax')(temporal_attention)
    temporal_attention = keras.layers.RepeatVector(int(width * 20))(temporal_attention)
    temporal_attention = keras.layers.Permute([2, 1])(temporal_attention)

    temporal_attention_result = keras.layers.multiply([spatial_attention_result, temporal_attention])
    # Do not do the addition, flatten instead
    temporal_attention_result = keras.layers.Lambda(lambda xin: tf.keras.backend.sum(xin, axis=-2), output_shape=(int(width * 20),))(temporal_attention_result)
    # flattened_sequence = keras.layers.Flatten()(temporal_attention_result)

    # Softmax layer to make the decision
    softmax_gesture_detection = keras.layers.Dense(2, activation=tf.nn.softmax,
                                                          name='softmax_gesture_detection')(temporal_attention_result)

    softmax_gesture_type = keras.layers.Dense(N_CLASSES, activation=tf.nn.softmax, name='softmax_gesture_type')(
        temporal_attention_result)

    model = keras.models.Model(inputs=[input_fft, input_gyro, input_lin_accel],
                               outputs=[softmax_gesture_detection, softmax_gesture_type])
