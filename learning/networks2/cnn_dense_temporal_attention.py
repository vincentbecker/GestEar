...
def create_model(n_fft, width):
    ...
    temporal_attention = keras.layers.Dense(1, activation=tf.nn.tanh)(time_distributed)
    ...
    temporal_attention_result = keras.layers.multiply([time_distributed, temporal_attention])
    # Do not do the addition, flatten instead
    temporal_attention_result = keras.layers.Lambda(lambda xin: tf.keras.backend.sum(xin, axis=-2), output_shape=(int(width * 20),))(
    ...
    softmax_gesture_detection = keras.layers.Dense(2, activation=tf.nn.softmax,
                                                          name='softmax_gesture_detection')(
        temporal_attention_result)

    softmax_gesture_type = keras.layers.Dense(N_CLASSES, activation=tf.nn.softmax,
                                                     name='softmax_gesture_type')(
        temporal_attention_result)
